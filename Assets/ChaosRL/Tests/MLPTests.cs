...
{
    public class MLPTests : TensorScopedTestBase
    {
        //------------------------------------------------------------------
        [Test]
        public void MLPTensor_SimpleClassification_LearnsMapping()
        {
            ...
            using var mlp = new MLP( numInputs: 4, layerSizes: new[] { 4, 1 }, lastLayerNonLin: false );
            ...
            using var inputs = new Tensor( new[] { 2, 4 }, inputData, "inputs" );
            using var targets = new Tensor( new[] { 2, 1 }, targetData, "targets" );

            // Create optimizer
            using var optimizer = new AdamOptimizer( new[] { mlp.Parameters }, beta1: 0.9f, beta2: 0.999f );
            ...
            for (int epoch = 0; epoch < epochs; epoch++)
            {
                ...
                // Forward pass
                using var predictions = mlp.Forward( inputs );

                // Compute MSE loss: mean((predictions - targets)^2)
                using var diff = predictions - targets;
                using var squared = diff * diff;
            ...
            mlp.ZeroGrad();
            using var finalPredictions = mlp.Forward( inputs );
